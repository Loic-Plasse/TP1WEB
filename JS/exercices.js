...
function Appeler_Range()
{
	...
	const start = Number(document.forms["range"]["start"].value) || 0;
	// variable step prend la valeur 1 par défaut
	const step = Number(document.forms["range"]["step"].value) || 1;
	
	// appel de la fonction Range
	let tab = Range(stop, start, step);
	
	// affichage du résultat dans la page html
	let resultat = document.getElementById("resultat_range").innerHTML = "Résultat : Range(" + stop + ", " + start + ", " + step + ") = " + tab;
	
}


// Fonction Range(stop, start=1, step=1)
// Retourne/Renvoie le tableau des entiers compris entre start (inclus) et 
// stop (exclu) avec un pas de step.
function Range(stop, start=1, step)
{
